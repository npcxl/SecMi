...
int sm2_z256_rand_range(sm2_z256_t r, const sm2_z256_t range)
{
	(void)r;
	(void)range;
	/* 随机数由上层 SM2 API 注入，此处不使用 GmSSL rand */
	return -1;
}
...
static void sm2_affine_generator(SM2_Z256_AFFINE_POINT *G)
{
	sm2_z256_from_hex(G->x, "32C4AE2C1F1981195F9904466A39C9948FE30BBFF2660BE1715A4589334C74C7");
	sm2_z256_from_hex(G->y, "BC3736A2F4F6779C59BDCEE36B692153D0A9877CC62A474002DF32E52139F0A0");
	sm2_z256_modp_to_mont(G->x, G->x);
	sm2_z256_modp_to_mont(G->y, G->y);
}

void sm2_z256_point_mul_generator(SM2_Z256_POINT *R, const sm2_z256_t k)
{
	SM2_Z256_AFFINE_POINT Ga;
	SM2_Z256_POINT Gj;
	sm2_affine_generator(&Ga);
	sm2_z256_point_copy_affine(&Gj, &Ga);
	sm2_z256_point_mul(R, k, &Gj);
}
...
/* sm2_z256_point_from_hash 已移除：依赖 GmSSL SM3 上下文 */
